...
if [[ $RESTIC_FTP_DOCKER_IS_FORGET_ENABLE == "1" || $RESTIC_FTP_DOCKER_IS_FORGET_ENABLE == "true" ]]; then
    echo "
# Forget script
$RESTIC_FTP_DOCKER_FORGET_CRON_SCHEDULE flock -n /opt/restic/forget.lockfile /opt/restic/forget.sh" >> /opt/restic/main.crontab
fi
